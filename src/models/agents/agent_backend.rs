...
use crate::helpers::general::{
    check_status_code, get_generated_project_path, read_code_template_contents,
...
impl AgentBackendDeveloper {
    ...
    async fn call_improved_backend_code(&mut self, factsheet: &mut FactSheet) {
        // Concatenate Instruction
        // At the following we have provided the whole factsheet in the project description
        let mut msg_context: String = format!(
            "CODE TEMPLATE: {:?}\n PROJECT_DESCRIPTION: {:?} \n",
    ...
    async fn call_extract_rest_api_endpoints(&self) -> String {
        let main_code = read_exec_main_contents();

        // Structured message context
        let msg_context: String = format!("CODE_INPUT: {:?}", main_code);
        ...
        while self.attributes.state != AgentState::Finished {
            match &self.attributes.state {
                ...
                AgentState::UnitTesting => {
                    ...
                    // Here we are running the cargo build command
                    // Note: we have copied the Cargo.toml file from the web_template project
                    // This means that the LLM is just generating the code (.rs) file and not the .toml file
                    let current_dir = get_generated_project_path();
                    dbg!(&current_dir);
                    ...
                    */
                    ...
                    // Convert API endpints into Values
                    ...
                    let mut run_backend_server: std::process::Child = Command::new("cargo")
                        .arg("run")
                        .current_dir(get_generated_project_path())
                        .stdout(Stdio::piped())
                        .stderr(Stdio::piped())
                        .spawn()
                    ...
                    for endpoint in check_endpoints {
                        ...
                        match check_status_code(&client, &url).await {
                            ...
                            Err(e) => {
                                // To kill a process which is running on port 8080
                                // kill $(lsof -t -i:8080)
                                run_backend_server
                                    .kill()
...
mod tests {
    ...
    async fn tests_writing_backend_code() {
        ...
        // "project_description": "build a website that only tracks time of the day

        let factsheet_str = r#"
    {
        "project_description": "build a website that prints a welcome greeting msg with a quote. The quote should be different each time.",
        "project_scope": 
             {
